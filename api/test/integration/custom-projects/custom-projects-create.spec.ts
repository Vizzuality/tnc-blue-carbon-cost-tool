...
describe('Create Custom Projects - Setup', () => {
  ...
  describe('Restoration project', () => {
    test('temp test to set restoration plan', async () => {
    ...
    test('Should fail if the total restored area is greater than the project size', async () => {
      const { parameters, ...rest } = RestorationCreateCustomProjectDTO;
      rest.projectSizeHa = 1000;
      const yearlyBreakdown = [
        {
          year: -1,
      ...
      expect(response.body.errors[0].title).toEqual(
        'Restoration plan exceeds project size',
      );
    });
  });
});
