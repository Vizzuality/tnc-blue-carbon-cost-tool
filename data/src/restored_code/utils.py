...
# Internal filename: /home/elena/vizzuality/repos/tnc-blue-carbon-cost-tool/data/notebooks/../src/utils.py
# Bytecode version: 3.12.0rc2 (3531)
# Source timestamp: 2024-10-23 09:27:40 UTC (1729675660)
...
def get_value_from_master_table(master_table, country_code, ecosystem, column):
    ...
    return master_table.loc[(master_table['country_code'] == country_code) & (master_table['ecosystem'] == ecosystem), column].values[0]
...
def calculate_npv(cost_plan, discount_rate, actual_year=-4):
    ...
    for year, cost in cost_plan.items():
        if year == actual_year:
            ...
            npv += cost / (1 + discount_rate) ** (year + (-actual_year - 1))
        else:
            npv += cost / (1 + discount_rate) ** (-actual_year + year)
    return npv

def calculate_cost_plan(base_size_df, base_increase_df, project_size_ha, base_key, base_cost, activity, ecosystem):
    """
    ...
    increased_by = float(base_increase_df.loc[base_increase_df['ecosystem'] == ecosystem, base_key].values[0])
